...
namespace Config
{
	...
	void Init()
	{
		...
		// Vita: feste interne Render-/Fenstergröße; Config.txt darf dies NICHT überschreiben
		renderwidth  = 452;
		renderheight = 256;
		windowwidth  = 960;
		windowheight = 544;

		// Vita: FOV/Focal Length fix auf 192
		focallength = 192;
		...
		if (file.is_open())
		{
			while (!file.eof())
			{
				...
				if (line.size() && (line[0] != ';'))
				{
					...
					{
						// Vita: ignorieren, Rendergröße bleibt auf festen Defaults
						// (Werte aus PC-Configs werden absichtlich nicht übernommen)
					}
					if (command == "windowsize")
					{
						// Vita: ignorieren, Fenstergröße bleibt 960x544
					}
					if (command == "focallength")
					{
						// Vita: FOV/Focal Length bleibt fix auf 192, Wert aus config.txt wird ignoriert
						// int ignored = std::stoi(line);
					}
		...
		// Load overlay/effects settings from config.txt (KEY=VALUE)
		EffectsConfigInit();
	...
	void Save()
{
    ...
    if (file.is_open())
    {
        ...
        // KEINE RENDERSIZE/WINDOWSIZE/FOCAL_LENGTH mehr
        ...
        // NOTE: maxweapon wird absichtlich nicht persistiert

        file << "\n;RightStickDeadzone\n";
        file << "rightStickDeadzone " << (int)rightStickDeadzone << "\n";
        file << "\n;LeftStickDeadzone\n";
        file << "leftStickDeadzone " << (int)leftStickDeadzone << "\n";
	...
	static void LoadNormalizedKeys()
	{
		...
		while (std::fgets(buf, sizeof(buf), f))
		{
			...
			*eq = '\0';
			...
			// Vita: jegliche Render-/Fenstergrößen aus config.txt ignorieren
			if (ieq(key, "RENDERSIZE") || ieq(key, "WINDOWSIZE"))
			{
				int dummyW = 0, dummyH = 0;
				parse_wh(val, dummyW, dummyH); // Werte „auffuttern“, aber nicht anwenden
			}
			...
			{
				if (*val >= '0' && *val <= '9')
					rightStickDeadzone = (char)parse_int(val);
				else if (*val)
					rightStickDeadzone = *val;
			}
			else if (ieq(key, "LS_DEADZONE") || ieq(key, "LEFTSTICKDEADZONE"))
			{
				if (*val >= '0' && *val <= '9')
					leftStickDeadzone = (char)parse_int(val);
				else if (*val)
					leftStickDeadzone = *val;
			}
	...
	static void NormalizeConfigFileToNewStyle()
{
    ...
    char keys_line[256] = {0};
    {
        ...
        while (std::fgets(buf, sizeof(buf), f))
        {
            if (strncmp(buf, "keys ", 5) == 0 || strncmp(buf, "keys\t", 5) == 0 ||
                strncmp(buf, "KEYS ", 5) == 0 || strncmp(buf, "KEYS\t", 5) == 0)
            {
    ...
    // KEINE RENDERSIZE/WINDOWSIZE/FOCAL_LENGTH-Ausgabe mehr
    // (renderwidth/renderheight/windowwidth/windowheight/focallength sind intern fix)
