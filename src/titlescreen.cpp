...
void TitleScreen::Render(SDL_Surface *src, SDL_Surface *dest, Font &font)
{
	...
	{
		font.PrintMessage("PSVITA PORT", 70, dest, 1);
		font.PrintMessage("BY JETSTREAMSHAM", 80, dest, 1);

		font.PrintMessage("ADDITIONAL PROGRAMMING", 95, dest, 1);
		font.PrintMessage("LAUNCHER CHEATMENU AND OPTIMIZING", 105, dest, 1);
		font.PrintMessage("BY ANDIWELI", 115, dest, 1);

		font.PrintMessage("VITA2D PORT", 130, dest, 1);
		font.PrintMessage("BY ANDIWELI", 140, dest, 1);

		font.PrintMessage("BASED ON ZGLOOM BY SWIZPIG", 160, dest, 1);
	}
